...
def get_periods(date_start=None, date_end=None, nb_period=1, period_type=None, full_periods=True):
    ...
    if not date_start and not date_end:
        date_end = datetime.datetime.now()
    if date_start:
        date_start = reset_period(date_start, period_type=period_type) if full_periods else date_start
        dates = [add_period(date_start, i, period_type=period_type) for i in range(nb_period)]
    elif date_end:
        date_end = reset_period(date_end, period_type=period_type) if full_periods else date_end
